...
# In this exercise, we will employ the topics reviewed in the lectures as well as a few new techniques
# using the September 2013 version of this rich, nationally representative dataset (available online).
# 
# The observations in the dataset represent people surveyed in the September 2013 CPS 
# who actually completed a survey. While the full dataset has 385 variables, in this exercise 
# we will use a more compact version of the dataset, CPSData.csv, which has the following variables:
#         
...
#          (missing if the interviewee does not live in a metropolitan area). The mapping from codes 
#          to names of metropolitan areas is provided in the file MetroAreaCodes.csv.
# 
...
# 
# Recall from the homework assignment "The Analytical Detective" that you can call the sort() function
# on the output of the table() function to obtain a sorted breakdown of a variable. 
...
0.88832615 + 0.05386818 # A: 0.9421943
...
# Black, Pacific Islander, White, or Multiracial) and ethnicity. 
...
# in the CPS dataset of Hispanic ethnicity? (Select all that apply.)
...
CPS %>%
        ...
        summarise(n=n()) %>%
        arrange(Hispanic, n) %>%
        filter(Hispanic == 1, n >=250)
        # A: American Indian, Multiracial, Black, White
# or
table(CPS$Race, CPS$Hispanic) # not so clear
...
# 
# Often when evaluating a new dataset, we try to identify if there is a pattern in the missing values
#in the dataset. We will try to determine if there is a pattern in the missing values 
#of the Married variable. The function is.na(CPS$Married) returns a vector of TRUE/FALSE values
#for whether the Married variable is missing. We can see the breakdown of whether Married is missing
...
table(CPS$Age, is.na(CPS$Married)) #! missing values
...
# does not live in a metropolitan area. Using the same technique as in the previous question,
...
# Again, treat the District of Columbia as a state.
        # A: 3 - District of Columbia, New Jersey, Rhode Island
...
table(CPS$Region, is.na(CPS$MetroAreaCode)) # Calculate proportions manually - hard

CPS %>%
        select(Region, MetroAreaCode) %>%
        group_by(Region, is.na(MetroAreaCode)) %>%
        summarise(n=n()) %>%
        mutate(freq=n/sum(n)) %>%
        arrange(`is.na(MetroAreaCode)`, freq)
        # A: Midwest


# Problem 2.5 - Evaluating Missing Values
# 
# While we were able to use the table() command to compute the proportion
# of interviewees from each region not living in a metropolitan area,
# it was somewhat tedious (it involved manually computing the proportion for each region)
# and isn't something you would want to do if there were a larger number of options.
# It turns out there is a less tedious way to compute the proportion of values that are TRUE.
# The mean() function, which takes the average of the values passed to it, will treat TRUE as 1
# and FALSE as 0, meaning it returns the proportion of values that are true. 
...
t <- tapply(is.na(CPS$MetroAreaCode), CPS$State, mean) 
# Ignore Alaska, Wyoming - all are non-metropolitan
sort(t)
        # A: Montana
...
#
# Codes like MetroAreaCode and CountryOfBirthCode are a compact way to encode factor variables
# with text as their possible values, and they are therefore quite common in survey datasets.
# In fact, all but one of the variables in this dataset were actually stored
# by a numeric code in the original CPS datafile.
# 
# When analyzing a variable stored by a numeric code, we will often want to convert it
# into the values the codes represent. To do this, we will use a dictionary, 
...
areas <- c("Atlanta-Sandy Springs-Marietta, GA",
           "Baltimore-Towson, MD",
           "Boston-Cambridge-Quincy, MA-NH",
           "San Francisco-Oakland-Fremont, CA")
CPS %>%
        filter(MetroArea %in% areas) %>%
        group_by(MetroArea) %>%
        summarise(n=n()) %>%
        arrange(desc(n))
        # A: Boston-Cambridge-Quincy, MA-NH
#or 
table(CPS$MetroArea) # much harder -> a lot of manual job
...
# of Hispanic ethnicity? Hint: Use tapply() with mean, as in the previous subproblem.
# Calling sort() on the output of tapply() could also be helpful here.
tapply(CPS$Hispanic, CPS$MetroArea, mean) %>% sort()
        # A: Laredo, TX
...
ma_20_and_more_Asians <-  sort(tapply(CPS$Race == "Asian", CPS$MetroArea, mean))
ma_20_and_more_Asians[ma_20_and_more_Asians >= 0.2] %>% length()
        # A: 4
...
# from each metropolitan area who have not received a high school diploma with the command:
#         
...
sort(tapply(CPS$Education == "No high school diploma", CPS$MetroArea,
            mean, na.rm=TRUE))
        # A: Iowa City, IA
...
table(CPS$MetroArea == "New York-Northern New Jersey-Long Island, NY-NJ-PA",
      CPS$Country != "United States")
        # Explanation: we can see that 1668 of interviewees from this metropolitan area
        # were born outside the United States and 3736 were born in the United States 
        # (it turns out an additional 5 have a missing country of origin). 
...
# Explanation: To obtain the number of TRUE values in a vector of TRUE/FALSE values, 
# you can use the sum() function. For instance, sum(c(TRUE, FALSE, TRUE, TRUE)) is 3. 
# Therefore, we can obtain counts of people born in a particular country 
# living in a particular metropolitan area with:
#       sort(tapply(CPS$Country == "India", CPS$MetroArea, sum, na.rm=TRUE))
#       sort(tapply(CPS$Country == "Brazil", CPS$MetroArea, sum, na.rm=TRUE))
#       sort(tapply(CPS$Country == "Somalia", CPS$MetroArea, sum, na.rm=TRUE))
# We see that New York has the most interviewees born in India (96),
# Boston has the most born in Brazil (18),
# and Minneapolis has the most born in Somalia (17).
