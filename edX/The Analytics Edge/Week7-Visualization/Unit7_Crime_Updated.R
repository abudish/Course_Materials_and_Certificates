...
# Removing red squares on the water
LatLonCounts2 <- subset(LatLonCounts, subset=LatLonCounts$Freq > 0)
ggmap(chicago) + geom_tile(data = LatLonCounts2, aes(x = Long, y = Lat, alpha = Freq), fill="red")
nrow(LatLonCounts) - nrow(LatLonCounts2) #952
...
# Redo the plot, Redo the map from Video 6, but this time fill each state with the variable GunOwnership. This shows the percentage of people in each state who own a gun.
