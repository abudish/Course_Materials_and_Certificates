...
# hospitalizations, and deaths. According to the National Vital Statistics Reports published in October 2012,
...
# (EISS) detect influenza activity through virologic and clinical data, including Influenza-like Illness
# (ILI) physician visits. Reporting national and regional data, however, are published with a 1-2 week lag.
# 
...
#         
# ILI Data - The CDC publishes on its website the official regional and state-level percentage of patient
# visits to healthcare providers for ILI purposes on a weekly basis.
# 
# Google Search Queries - Google Trends allows public retrieval of weekly counts for every query 
# searched by users around the world. For each location, the counts are normalized 
# by dividing the count for each query in a particular week by the total number of online search queries
# submitted in that location during the week. Then, the values are adjusted to be between 0 and 1.
# 
...
# be a good modeling choice. Based on our understanding of the data 
# from the previous subproblem, which model best describes our estimation problem?
...
cor(FluTrain$Queries, log(FluTrain$ILI))^2
        # A: R-squared = Correlation^2


# Problem 3.1 - Performance on the Test Set
# 
# The csv file FluTest.csv provides the 2012 weekly data of the ILI-related search queries and the
# observed weekly percentage of ILI-related physician visits. 
...
# (HINT: You can either just output FluTest$Week to find which element corresponds to March 11, 2012,
# or you can use the "which" function in R. 
# To learn more about the which function, type ?which in your R console.)
index <- grep('2012-03-11', FluTest$Week)
PredTest1[index]
        # A: 2.187378
...
(FluTest[index, 'ILI'] - PredTest1[index])/FluTest[index, 'ILI']
        # A: 0.04623827
...
# Often, statistical models can be improved by predicting the current value of
# the dependent variable using the value of the dependent variable from earlier weeks.
...
# Because the ILI variable is reported with a 1- or 2-week lag, a decision maker cannot rely
# on the previous week's ILI value to predict the current week's value. Instead,
# the decision maker will only have data available from 2 or more weeks ago.
# We will build a variable called ILILag2 that contains the ILI value from 2 weeks before
# the current observation.
# 
# To do so, we will use the "zoo" package, which provides a number of helpful methods
# for time series models. While many functions are built into R, you need to add new packages
# to use some functions. New packages can be installed and loaded easily in R,
# and we will do this many times in this class. 
...
library(zoo)
# After installing and loading the zoo package, run the following commands 
# to create the ILILag2 variable in the training set:
# stats::lag - because lag is also in dplyr
...
# of our dataset, where we can't compute the data from 2 weeks earlier.
# 
# How many values are missing in the new ILILag2 variable?
FluTrain %>% filter(is.na(ILILag2)) %>% nrow
...
# 
# Train a linear regression model on the FluTrain dataset to predict the log 
# of the ILI variable using the Queries variable as well 
# as the log of the ILILag2 variable. Call this model FluTrend2.
...
# 
# Modify the code from the previous subproblem to add an ILILag2 variable 
# to the FluTest data frame. How many missing values are there in this new variable?
...
# the training set contains all observations from 2004-2011 and the testing set
# contains all observations from 2012. There is no time gap between the two datasets,
# meaning the first observation in FluTest was recorded one week after the last 
# observation in FluTrain. From this, we can identify how to fill in the missing
# values for the ILILag2 variable in FluTest.
# 
# Which value should be used to fill in the ILILag2 variable 
# for the first observation in FluTest?
...
# It may be helpful to check the total number of rows in FluTrain 
...
# again remembering to call the exp() function on the result of the predict() function
# to obtain predictions for ILI instead of log(ILI).
# 
