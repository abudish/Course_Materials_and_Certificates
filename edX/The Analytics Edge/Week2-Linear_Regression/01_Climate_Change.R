...
#   Temp: the difference in degrees Celsius between the average global temperature in that period
#        and a reference value.
#   CO2, N2O, CH4, CFC.11, CFC.12: atmospheric concentrations of carbon dioxide (CO2), nitrous oxide (N2O),
#      methane  (CH4), trichlorofluoromethane (CCl3F; commonly referred to as CFC-11) 
#      and dichlorodifluoromethane (CCl2F2; commonly referred to as CFC-12), respectively.
#   CO2, N2O and CH4 are expressed in ppmv (parts per million by volume  -- i.e., 397 ppmv of CO2 means
...
#      area). Due to sunspots and other solar phenomena, the amount of energy that is given off by the sun
#      varies substantially with time.
#   MEI: multivariate El Nino Southern Oscillation index (MEI), a measure of the strength of 
#      the El Nino/La Nina-Southern Oscillation (a weather effect in the Pacific Ocean that affects 
#      global temperatures).
...
CLM_change <- read.csv('climate_change.csv')

# Then, split the data into a training set, consisting of all the observations
# up to and including 2006, 
# and a testing set consisting of the remaining years (hint: use subset).
# A training set refers to the data that will be used to build the model 
# (this is the data we give to the lm() function), 
# and a testing set refers to the data we will use to test our predictive ability.
library(dplyr)
train <- CLM_change %>% filter(Year <= 2006)
test <- CLM_change %>% filter(Year > 2006)
...
summary(model1)
        # A: all included, except N20 and CH4
...
# Which of the following is the simplest correct explanation for this contradiction?
        #A: All of the gas concentration variables reflect human development - 
...
# This trade-off is formalized by the Akaike information criterion (AIC)
# - it can be informally thought of as the quality of the model with a penalty
...
# with the full model as the initial model
# (HINT: If your initial full model was called "climateLM",
...
# by the step function? Select all that apply.
        #A: CH4
...
# necessarily produce a very interpretable model - just a model that has
# balanced quality and simplicity for 
...
# to the training data, but does the model quality hold when applied to unseen data?
# 
...
temp_predictions <- predict(model_AIC, newdata = test)
# Enter the testing set R2:
SSE = sum((temp_predictions - test$Temp)^2)
