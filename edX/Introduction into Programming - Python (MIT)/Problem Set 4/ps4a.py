...
def getWordScore(word, n):
    ...
    sumForTheWord = 0
    for letter in word:
        sumForTheWord += SCRABBLE_LETTER_VALUES[letter]
    wordScore = 0
    if len(word) == n:
        wordScore = sumForTheWord * len(word) + 50
        return wordScore
    else:
        wordScore = sumForTheWord * len(word)
        return wordScore
    ...
    for letter in word:
        if letter in handCopy:
            ...
            if handCopy[letter] >= 0:
                counter += 1
    return counter == len(word) and word in wordList
